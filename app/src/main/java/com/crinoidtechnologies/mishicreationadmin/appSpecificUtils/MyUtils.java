...
public class MyUtils {
    public static final String COPY_MESSAGE_LABLE = "copy_label";
...
//    }
    ...
    public static void copyTextToClipBoard(Context context, String text) {
        ...
        manager.setPrimaryClip(ClipData.newPlainText(COPY_MESSAGE_LABLE, text));
    }
...
//                }
    ...
    public static void changeColorOfMenuItems(Menu menu, int colorCode) {
        for (int i = 0; i < menu.size(); i++) {
            ...
            if (drawable != null) {
                drawable.mutate();
                drawable.setColorFilter(MyApplication.getAppContext().getResources().getColor(colorCode), PorterDuff.Mode.SRC_ATOP);
            }
        }
    }
    ...
    public static void changeColorOfButtonBackground(View button, int colorCode) {
        if (Build.VERSION.SDK_INT < 20) {
            button.setBackgroundColor(ContextCompat.getColor(MyApplication.getAppContext(),colorCode));
        } else {
            button.getBackground().setColorFilter(ContextCompat.getColor(MyApplication.getAppContext(),colorCode), PorterDuff.Mode.SRC_ATOP);
        }
    }
